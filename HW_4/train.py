...
def plot_losses(train_losses: List[float], val_losses: List[float]):
    ...
    """
    train_perplexities, val_perplexities = [torch.exp(torch.tensor(loss)) for loss in train_losses], [torch.exp(torch.tensor(loss)) for loss in val_losses]
...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        # logits: (B, L, V), indices: (B, L) - need to transpose logits
